...
public class AdocReporter implements Reporter {
    ...
    public void write(LogStatistics stats, Path outputPath) {
        try (PrintWriter writer = new PrintWriter(Files.newBufferedWriter(outputPath))) {
            ...
            writer.printf(
                    "Средний размер ответа:: %,.2f b%n",
                    stats.getResponseSizeInBytes().getAverage());
            writer.printf(
                    "95-й перцентиль:: %,.2f b%n",
            ...
            for (LogStatistics.ResourceCount rc : stats.getResources()) {
                writer.printf("| `%s` | %,d%n", rc.getResource(), rc.getTotalRequestsCount());
            }
            writer.println("|===");
            ...
            for (LogStatistics.ResponseCodeCount c : stats.getResponseCodes()) {
                writer.printf(
                        "| %d | %s | %,d%n", c.getCode(), statusCodeToName(c.getCode()), c.getTotalResponsesCount());
            }
            writer.println("|===");
            ...
            throw new RuntimeException("Не удалось записать AsciiDoc-отчёт: " + e.getMessage(), e);
        }
    }
