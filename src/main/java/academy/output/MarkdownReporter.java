...
public class MarkdownReporter implements Reporter {
  ...
  public void write(LogStatistics stats, Path outputPath) {
    try (PrintWriter writer =
      ...
      writer.printf("| Средний размер ответа |      %,.2fb |\n",
                    stats.getResponseSizeInBytes().getAverage());
      writer.printf("|  95p размера ответа   |      %,.2fb |\n",
      ...
      codes.sort((a, b)
                     -> Integer.compare(b.getTotalResponsesCount(),
                                        a.getTotalResponsesCount()));
      ...
      for (LogStatistics.RequestsPerDate d : stats.getRequestsPerDate()) {
        writer.printf("| %-12s | %-13s | %,11d | %7.2f%% |\n", d.getDate(),
      ...
      throw new RuntimeException(
          "Не удалось записать Markdown-отчёт: " + e.getMessage(), e);
    }
  }
