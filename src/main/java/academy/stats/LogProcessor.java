...
public class LogProcessor {
  ...
  public LogProcessor(List<String> sourcePaths, LocalDate fromDate,
                      LocalDate toDate) {
  ...
  public LogStatistics process() {
    ...
    for (String path : sourcePaths) {
      ...
      try (LogSource source = LogSourceFactory.from(path)) {
        source.lines()
            .peek(line
                  -> logger.trace("Строка из источника '{}': {}", path, line))
            .map(parser::parse)
            .filter(Optional::isPresent)
            .map(Optional::get)
            .filter(
                entry -> isWithinDateRange(entry.getTimestamp().toLocalDate()))
            .forEach(allEntries::add);
      } catch (Exception e) {
        logger.error("Ошибка при обработке источника {}: {}", path,
                     e.getMessage(), e);
        System.exit(2);
      }
    }
    ...
    if (allEntries.isEmpty()) {
      logger.warn("Ни одна запись не была распарсена из {} источников",
                  sourcePaths.size());
    ...
    List<Long> responseSizes = allEntries.stream()
                                   .map(LogEntry::getResponseSizeBytes)
                                   .collect(Collectors.toList());

    double avg =
        responseSizes.stream().mapToLong(Long::longValue).average().orElse(0.0);
    double max =
        responseSizes.stream().mapToLong(Long::longValue).max().orElse(0L);
    ...
    Map<String, Long> resourceCounts = allEntries.stream().collect(
        Collectors.groupingBy(LogEntry::getResource, Collectors.counting()));
    List<LogStatistics.ResourceCount> topResources =
        resourceCounts.entrySet()
            .stream()
            .sorted(Map.Entry.<String, Long>comparingByValue().reversed())
            .limit(TOP_RESOURCES_LIMIT)
            .map(e -> {
              LogStatistics.ResourceCount rc =
                  new LogStatistics.ResourceCount();
    ...
    Map<Integer, Long> codeCounts = allEntries.stream().collect(
        Collectors.groupingBy(LogEntry::getStatusCode, Collectors.counting()));
    List<LogStatistics.ResponseCodeCount> codeStats =
        codeCounts.entrySet()
            .stream()
            .map(e -> {
              LogStatistics.ResponseCodeCount c =
                  new LogStatistics.ResponseCodeCount();
              c.setCode(e.getKey());
    ...
    List<String> uniqueProtocols = allEntries.stream()
    ...
    Map<LocalDate, Long> dateCounts =
        allEntries.stream().collect(Collectors.groupingBy(
            e -> e.getTimestamp().toLocalDate(), Collectors.counting()));
    ...
    List<LogStatistics.RequestsPerDate> dateStats =
        dateCounts.entrySet()
            .stream()
            .map(e -> {
              ...
              LogStatistics.RequestsPerDate dpd =
                  new LogStatistics.RequestsPerDate();
              dpd.setDate(date.format(DATE_FORMATTER));
              dpd.setWeekday(date.getDayOfWeek().getDisplayName(
