...
@Command(name = "log-analyzer", version = "Log Analyzer 1.0",
...
public class Application implements Callable<Integer> {
  ...
  @Option(names = {"--path", "-p"}, required = true,
          description = "Путь к лог-файлам")
  private List<String> paths;

  @Option(names = {"--format", "-f"}, required = true,
  ...
  @Option(names = "--from",
          description = "Начальная дата (ISO8601, например: 2025-03-01)")
  private String fromStr;

  @Option(names = "--to",
          description = "Конечная дата (ISO8601, например: 2025-03-31)")
  ...
  private void validateOutputFile() {
    if (Files.exists(output)) {
      throw new IllegalArgumentException("Файл отчёта уже существует: " +
                                         output);
    }
    ...
    if (("json".equals(fmt) && !".json".equals(ext)) ||
      ...
      throw new IllegalArgumentException(
          "Несоответствие формата и расширения: " + format + " → " + ext);
    }
    ...
    if (!Files.isWritable(parent)) {
      throw new IllegalArgumentException("Директория недоступна для записи: " +
                                         parent);
    }
  }
  ...
  private void validateLocalPath(String path) {
    ...
    if (!name.endsWith(".log") && !name.endsWith(".txt")) {
      throw new IllegalArgumentException(
          "Файл должен иметь расширение .log или .txt: " + path);
    }
  }
  ...
  private void checkUrlExists(String url) {
    try {
      ...
      if (response.statusCode() >= 400) {
        throw new IllegalArgumentException(
            "Удалённый файл недоступен: " + url +
            " (статус: " + response.statusCode() + ")");
      }
    } catch (IOException | InterruptedException e) {
      throw new IllegalArgumentException("Недоступный URL: " + url + " (" +
                                         e.getMessage() + ")");
    }
  }
